"""
타이타닛 데이터셋 기초 탐색
pandas head/tail/shape/info/columns를 사용해서 데이터셋의 기본 정보를 화면에 순서대로 보여주는 streamlit 앱이다.
실행방법 streamlit run 9_7_1.py
"""
...
# st.file_uploader로 파일을 직접 올릴 수 있다, 아무것도 올리지 않으면 아래의 기존 로직대로 같은 폴더 타이타닉 파일을 찾아서 읽는다.
uploade_file = st. file_uploader("Titanic_csv 파일을 직접 업로드 할 수 있습니다.(선택사항)", type="csv")

if uploade_file is not None : 
    df = pd.read_csv(uploade_file)
else :
    try :
        # 이 스크립트와 같은 폴더에 있는 타이타닉 폴더를 읽어 온다.
        df = pd.read_csv(CSV_PATH)

    except FileNotFoundError :
        #파일이 없을 때 사용자가 무억을 해야 하는지 화면에 안내 한다.
